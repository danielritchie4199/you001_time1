  ...
  try {
        ...
        try {
          ...
          if (detailError.message.includes('quota') || detailError.message.includes('quotaExceeded')) {
            ...
            if (newDetailKey) {
              ...
              throw new Error('ALL_QUOTA_EXCEEDED');
            }
          } else {
            throw detailError;
          }
    ...
    if (searchResults.length > 0) {
      try {
        ...
        console.error('Elasticsearch 인덱싱 실패:', esError.message);
      }
    }
    ...
    res.status(500).json({
      success: false,
      error: error.message,
      searchDuration: `${errorDurationSeconds}초`
    });
  }
});
...
function getVideoLengthCategory(durationInSeconds) {
  ...
  if (durationInSeconds < 3600) return 'long3';
  return 'long4';
}
...
// Rate Limiting은 이미 위에서 구현됨 (중복 제거)

function rateLimitMiddleware_duplicate1(req, res, next) {
  const clientIP = req.ip || req.connection.remoteAddress;
  const now = Date.now();
...
// Express 앱과 미들웨어는 이미 위에서 초기화됨 (중복 제거)
...
// 서버 시작
app.listen(PORT, () => {
  console.log(`🚀 YouTube 검색 서버가 포트 ${PORT}에서 실행 중입니다.`);
  console.log(`📱 웹 인터페이스: http://localhost:${PORT}`);
  console.log(`🔍 API 엔드포인트: http://localhost:${PORT}/api/search`);
  
  // Elasticsearch 연결 상태 출력
  if (esClient) {
    console.log('🔍 Elasticsearch 연결 상태: 연결됨');
  } else {
    console.log('⚠️  Elasticsearch 연결 상태: 연결 안됨');
  }
});
